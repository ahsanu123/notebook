...
#pagebreak()
== Raw Matrix Reading To KeyMap Function <raw_mat_read_and_keymap>
...
== Raw Matrix, Key Manager and KeyState 

note about Raw Matrix, Key Manager and KeyState.

=== Raw Matrix to Key Manager

#let rawMatrix = "Raw Matrix"
#let keyManager = "Key Manager"
#let stateMatrix = "State Matrix"
#let eventHandlers = "Event Handlers"
...
#chronos.diagram({
  ...
  _seq(rawMatrix, rawMatrix, comment: [scan and\ merge raw\ matrix])
  _seq(rawMatrix, keyManager , comment: [send merged\ raw data to\ key manager])
  _seq(keyManager, keyManager , comment: [map raw data\ to key event\ by using keymap\ look at @raw_mat_read_and_keymap])

  _note("over", [state matrix\ will contain buffer\ for max length\ of key which\ about\ ROW_NUM \* COL_NUM], pos: (stateMatrix))

  _seq(keyManager, stateMatrix, comment: [save data to\ state matrix])
  _seq(stateMatrix, eventHandlers, comment: [handle event])
})

=== Key State Event Handler

note about how main key event handler work

#pseudocode-list(title: [#appleRed Key Event Handler], booktabs: true)[
  + enum *eRetentionType*
    + Toggle
    + Arbitrary
  + *end*
  ...
  + struct *KeyState*
    + char: *u8*
    + time: *timestamp*
    + retentionType: *eRetentionType*
    + keyType: *eKeyType*
  + *end*
  + \
  + 
  + let stateMatrix = *state matrix generated by @raw_mat_read_and_keymap*
  + \
  + *foreach* state in *stateMatrix*
    + rowPins[row].setHigh()
    + let columnState = *read all column pin state*
    + matrixResult.push(columnState)
  + *end*
...
== Sudi Finite State Machine 

- IDL = idle
- ONE = one click 
- SND = send 
- DBL = double click 
- DBH = double click and hold
...
#automaton(
  ...
  (
   ...
   SND: (IDL: none),
   DBL: (SND: none),
   DBH: (DBH: none, SND: none),
   ONE: (SND: none, DBL:none, DBH: none),
 ))
